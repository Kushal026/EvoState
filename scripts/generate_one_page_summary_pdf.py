...
def build_pdf(filename="one_page_concept_summary.pdf"):
    ...
    header_data = [
        ...
        [
            Paragraph("One-Page Scientific Concept &amp; Evaluation Summary | Primary Sources: Kosowski et al. (2025), Engdahl et al. (2026), Gu &amp; Dao (2023), Snell et al. (2024)", subtitle_style),
            Paragraph("<b>Status:</b> Verified &amp; Reproducible", ParagraphStyle('Status', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=6.5, leading=8, textColor=colors.HexColor("#16a34a"), alignment=TA_RIGHT))
        ]
    ]
    header_table = Table(header_data, colWidths=[4.4*inch, 3.2*inch])
    ...
    col1_content = [
        ...
        Paragraph("• <b>Inference-Time Scaling (Latent Deliberation):</b> Test-time refinement optimizes query sharpening without updating model weights: v^{(k+1)} = (1-η)v^{(k)} + η M_T q_{sharp}^{(k)}, helping de-noise superimposed coordinates.", body_style),
        Spacer(1, 3),
    ...
    arch_data = [
        [Paragraph("<b>Architecture</b>", table_header), Paragraph("<b>State Space</b>", table_header), Paragraph("<b>Step Latency</b>", table_header), Paragraph("<b>Interference Behavior</b>", table_header)],
        ...
        [Paragraph("Selective SSM (Mamba)", table_cell), Paragraph("O(d) Bounded", table_cell), Paragraph("O(1) Constant", table_cell), Paragraph("Decay over long horizons", table_cell)],
        [Paragraph("BDH (Dragon Hatchling)", table_cell), Paragraph("O(1) Bounded State", table_cell), Paragraph("O(1) Constant", table_cell), Paragraph("Evolving associative state", table_cell)],
        [Paragraph("BDH-CQ (Latent Reason)", table_cell), Paragraph("O(1) Bounded State", table_cell), Paragraph("O(K) Variable", table_cell), Paragraph("Test-time latent deliberation", table_cell)],
    ]
    arch_table = Table(arch_data, colWidths=[1.15*inch, 0.8*inch, 0.75*inch, 1.05*inch])
    ...
    col2_content = [
        ...
        Paragraph("• <b>Subspace Capacity Stress (N_pairs vs d=32):</b> Abrupt phase transition at N > 4 pairs, matching theoretical associative rank limits.", body_style),
        Paragraph("• <b>Test-Time Recovery:</b> Additional inference effort (K=1 to 16) yields measurable recovery when signal is in superposition, but cannot recover fully overwritten state.", body_style),
        Spacer(1, 3),
        Paragraph("<b>4. Evidence Classification Standard</b>", sec_header_style),
        Spacer(1, 1),
        Paragraph("• <font color='#2563eb'><b>[PUBLISHED]</b></font> BDH O(N) linear time &amp; state dynamics (Kosowski et al., 2025).", body_style),
        Paragraph("• <font color='#2563eb'><b>[PUBLISHED]</b></font> BDH-CQ in-context latent reasoning (Engdahl et al., 2026).", body_style),
    ...
    def_data = [
        [
            Paragraph("<b>CRITICAL BOUNDARIES, LIMITATIONS &amp; SCIENTIFIC DISTINCTIONS:</b>", ParagraphStyle('DefH', parent=styles['Normal'], fontName='Helvetica-Bold', fontSize=7, leading=8, textColor=colors.HexColor("#0f172a"))),
        ],
        [
            Paragraph(
                "<b>1. Educational Toy vs Published BDH:</b> Our browser toy is a pedagogical d=32 associative matrix surrogate. It does NOT implement official multi-billion parameter BDH weights. | "
                "<b>2. Biological Analogy:</b> Synapse/neuron mathematical duality is illustrative linear algebra; no biological claims are made. | "
                "<b>3. Live vs Precomputed:</b> Live experiments execute in PyTorch/TypeScript; heavy sweeps are labelled Precomputed with full seed transparency. | "
                "<b>4. Testable Hypothesis:</b> Tests whether test-time compute improves recovery without asserting universal scaling.",
    ...
    footer_text = (
        "<b>Primary References:</b> [1] Kosowski et al. (2025) <i>The Dragon Hatchling</i>, arXiv:2509.26507. "
        "[2] Engdahl et al. (2026) <i>BDH-CQ: In-Context Learning with Recurrent Latent Reasoning</i>, arXiv:2608.09888. "
        "[3] Gu &amp; Dao (2023) <i>Mamba</i>. [4] Snell et al. (2024) <i>Scaling LLM Test-Time Compute</i>. — <b>Code &amp; Data:</b> Open Source MIT / Deterministic Seeded (PyTorch 2.0+)"
    )
...
if __name__ == "__main__":
    ...
    build_pdf(out_path)
    # Also sync to frontend/public/docs
    ...
    if os.path.exists(dest_dir):
        ...
        print(f"Synced to {os.path.join(dest_dir, os.path.basename(out_path))}")
